    ...
    .then((problem) => {
      if (!problem) {
        return res
          .status(404)
          .send({
            message: "Reported problem not found with id : " + req.params.id,
          });
      }
      if (!problem.status) {
        return res.send({
          message:
            "The reported problem with id : " +
            req.params.id +
      ...
      Problem.deleteOne({ _id: req.params.id })
        .then((deleteProblem) => {
          res.send({
            message:
              deleteProblem.deletedCount +
              "Reported problem deleted successfully!",
          });
        })
        .catch((err) => {
          res.status(500).send({
        ...
        .then((problem) => {
          if (!problem) {
            return res
              .status(404)
              .send({
                message: "Problem not found with id specified " + req.params.id,
              });
          }
